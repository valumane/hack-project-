...
let is_oper(c : string) : bool =
  (c = "+") || (c = "-") || (c = "*") || (c = "/") || (c = "m")
;;

let val_number(c : string) : int =
  int_of_string(c)
;;

let apply_oper(c, v1, v2 : string * int * int) : int =
  if c = "+"
  then v1 + v2
  else
    if c = "-"
...
let chk_nodexpr(c,l,r: string*t_treexpr*t_treexpr):bool=
  if is_number(c)
...
let rec eval_treexpr_aux(t : t_treexpr) : int =
  let (c, l, r) : string * t_treexpr * t_treexpr =
    (bt_root(t), bt_subleft(t), bt_subright(t))
  in
  if chk_nodexpr(c, l, r)
  then
    if is_oper(c)
    then
      let v1 : int = eval_treexpr_aux(l) in
      let v2 : int = eval_treexpr_aux(r) in
      apply_oper(c, v1, v2)
    else val_number(c)
  else failwith("erreur eval_treexpr_aux : arbre mal forme")
;;
let eval_treexpr(t : t_treexpr) : int =
  if bt_isempty(t)
  then failwith("erreur eval_treexpr : arbre vide")
  else eval_treexpr_aux(t)
;;
